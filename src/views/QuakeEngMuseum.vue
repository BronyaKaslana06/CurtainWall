...
for (let i = 0; i < data_A.length; i++) {
  if (data_A[i].state === "1") uniqueSet.add(data_A[i].name[0]); // 将字母添加到Set对象中，重复的字母不会被重复添加
}
var sel_2_A = Array.from(uniqueSet);

var uniqueSet = new Set();
for (let i = 0; i < data_B.length; i++) {
  if (data_B[i].state === "1") uniqueSet.add(data_B[i].name[0]);
}
var sel_2_B = Array.from(uniqueSet);

var uniqueSet = new Set();
for (let i = 0; i < data_C.length; i++) {
  if (data_C[i].state === "1") uniqueSet.add(data_C[i].name[0]);
}
...
export default {
  ...
  watch: {
    ...
    setting_type: function () {
      ...
      if (that.setting_type === "measure") {
        ...
        that.$refs.unityModel.isMeasuring(
          that.setting_measure.is_open.toString()
        );
      } else {
  ...
  methods: {
    ...
    handleClick_compare() {
      //对比--开启对比按钮
      var that = this;
      if (!that.setting_compare.is_open) that.setting_compare.max_num = 1;
    },
    handleClick_reset() {
      //相机--重置按钮
      var that = this;
    ...
    handleClick_search(index) {
      //对比--显示特定位点    index>=0对比处search按钮  index=-1图像分割处search按钮
      var that = this;
      if (index >= 0) {
        //对比处search按钮
        var des = this.setting_compare.points[index];
        that.$refs.unityModel.specialize_des(
          des.info[0].data + "," + des.info[1].data + "," + des.info[2].data
        );
        ...
        that.$refs.unityModel.specialize_des(
          SCD.des.x + "," + SCD.des.y + "," + SCD.des.z
        );
      }
    },
    handleClick_delete(index) {
      //对比--删除特定位点
      var that = this;
      var des = this.setting_compare.points[index];
      that.$refs.unityModel.hide_des(
    ...
    handleClick_measure() {
      ...
      if (!that.setting_measure.is_open) {
        //关闭
        that.setting_measure.step = "0";
        that.setting_measure.has_data = false;
      }
    ...
    add_points(image_url, x, y, z, path) {
      //对比的信息列表    添加新元素+显示在模型上
      var that = this;
      var points = that.setting_compare.points;
      while (points.length >= that.setting_compare.max_num) {
        //已满，删去第一个         //在模型上隐藏位点
        that.$refs.unityModel.hide_des(
      ...
      if (!that.type_compare && len) {
        //逐个显示时,最后一个点位state为'1',倒数第二个为'2',其余均为'0'
        for (let i = 0; i < len; i++) {
          points[i].state = "0";
        }
    ...
    addSelections(index, datalist) {
      ...
      if (index === 1) {
        ...
        //记录在data_X中的位置,便于发送信息
        for (let i = 0; i < datalist.length; i++) {
          //0.a_004  (index.name)  =>  0.a_004  a_004
          that.selections[index].push({
            value: datalist[i],
            label: datalist[i].split(".")[1],
          });
        }
      }
    },
    updateSelections(index) {
      ...
      if (index === 0) {
        ...
        var sel_2 = sel_2_A;
        if (that.select_1 === "B") sel_2 = sel_2_B;
        else if (that.select_1 === "C") sel_2 = sel_2_C;
        ...
        var sel_3 = [];
        var datalist = [];
        if (that.select_1 === "A") datalist = data_A;
        else if (that.select_1 === "B") datalist = data_B;
        else if (that.select_1 === "C") datalist = data_C;

        for (let i = 0; i < datalist.length; i++) {
          if (
            datalist[i].name.startsWith(that.select_2) &&
            datalist[i].state === "1"
          ) {
            //a_004.JPG =>  0.a_004  (index.name)
    ...
    searchColorData(infoid) {
      console.log(
        "请求颜色信息：",
        `http://localhost:8443/api/color/${infoid}`
      );
      axios
        .get(`http://localhost:8443/api/color/${infoid}`)
        .then((response) => {
    ...
    sendSelections() {
      ...
      if (that.select_1 === "B") mess = data_B[index];
      else if (that.select_1 === "C") mess = data_C[index];
      //alert(mess.x + "," + mess.y + "," + mess.z);
      //this.$refs.unityModel.sendOrders(mess.x + "," + mess.y + "," + mess.z);

      var imageURL =
        "Pictures/DZGCG/source_image/" +
        that.select_1 +
        "/" +
        that.select_3.split(".")[1] +
        ".JPG";
      ...
      that.info[3].data =
        "/" + that.select_1 + "/" + that.select_3.split(".")[1] + ".JPG";

      var coloruId =
        that.select_1 +
        "_" +
        that.select_3.split(".")[1].replace(".JPG", "").replace("/", "_");
    ...
    updatePicture() {
      ...
      //修改格式
      var coloruId =
        that.info[3].data.split("/")[1] +
        "_" +
        that.info[3].data.split("/")[2].replace(".JPG", "").replace("/", "_");
      that.searchColorData(coloruId);
      // console.log("打印")
      // console.log(imageURL)
      that.url = imageURL;
    ...
    updateDistance() {
      //接收unity信息后更新测距步骤条与测距结果
      var that = this;
      if (that.unityMessage[0] === "1") {
        that.setting_measure.step = "1";
        that.setting_measure.has_data = false;
      } else if (that.unityMessage[0] === "2") {
        //'2,6,8,10/r'
    ...
    recieve(event) {
      ...
      if (event.data.type === "0") that.updatePicture();
    ...
    selectAllCracks() {
      this.StoneCrackDetect.onshow.no = this.StoneCrackDetect.block_data
        .map((block, index) =>
          block.has_crack
            ? this.StoneCrackDetect.onshow.options[index].value
            : null
        )
    ...
    selectAllRisk(risk_rank) {
      ...
      //查询全部风险图片id
      axios
        .get(`http://localhost:8443/api/risk/${risk_rank}`)
        .then((res) => {
          //存储全部风险图片id
          this.risk_setting.risk_data = res.data.map((item) => ({
            value: item,
            label: item,
          }));
        })
    ...
    selectRisk() {
      ...
      let csvData;
      if (select_1 == "B") csvData = data_B;
      else if (select_1 == "C") csvData = data_C;
      else csvData = data_A;
    ...
    <div class="details_and_compare">
        ...
        <div class="info">
          ...
          <a v-if="url">
            <el-button
              color="#B29F82"
              style="color: white"
              @click="handleClick_divide(url, info)"
            >
            ...
            <el-button
              color="#B29F82"
              style="color: white"
              @click="DoItYourself(info[3].data)"
              >送往检测系统</el-button
            >
          </a>
        </div>
            ...
            >
              ...
              >
                ...
                <div class="compare_operation">
                  ...
                  <a v-if="point.url">
                    <el-button
                      @click="handleClick_divide(point.url, point.info)"
                    >
                      <el-icon>
                        <Scissor />
                      </el-icon>
      ...
      <div v-if="StoneCrackDetect.is_show" class="divide">
        <el-card style="border-radius: 20px">
          <el-row>
            <el-col :span="8">
              ...
              <el-row>
                x: {{ StoneCrackDetect.des.x }}&emsp; y:
                {{ StoneCrackDetect.des.y }}&emsp; z:
                {{ StoneCrackDetect.des.z }}
              </el-row>
            </el-col>
            <el-col :span="8">
            ...
            <el-col :span="8">
              ...
              <a v-if="StoneCrackDetect.success">
                ...
                <el-row
                  >共分割得 &ensp;{{ StoneCrackDetect.seg_count }}&ensp;
                  块</el-row
                >
              </a>
            </el-col>
        ...
        <p class="text_2">
          ...
          <el-tooltip
            content="⽆⼈机采集图像的过程中，⽆⼈机与幕墙的物理距离保持⼀致。因此，以像素点作为裂缝测量的单位，数值具有可⽐性"
          >
            <el-button
              color="#B29F82"
              style="color: white"
              @click="selectAllCracks"
              >显示损伤幕墙块</el-button
            >
          </el-tooltip>
        </p>
        <div v-if="StoneCrackDetect.onshow.no[0]">
          <el-scrollbar height="500px">
            <el-row :gutter="20">
              <!-- 设置栅格间距 -->
              <el-col
                v-for="block in StoneCrackDetect.onshow.no"
                :key="block"
                :span="6"
              >
                <!-- 假设每张卡片占据6列 -->
                ...
                >
                  ...
                  <el-row
                    >NO.
                    {{
                      StoneCrackDetect.block_data[block].block_num + 1
                    }}
                    原图和检测结果</el-row
                  >
                  <el-row>
                      ...
                      <el-row>
                        <el-tooltip content="幕墙块图像">
                          <el-image
                            ...
                            :src="
                              StoneCrackDetect.block_data[block]
                                .block_seg_image_path
                            "
                            :zoom-rate="1.2"
                            :preview-src-list="[
                              StoneCrackDetect.block_data[block]
                  ...
                  <el-row style="width: 800px">
                    <el-col :span="7" style="margin-left: 1%; width: auto">
                      ...
                      <a v-if="StoneCrackDetect.block_data[block].has_crack">
                        <el-row
                          >所有裂缝像素⾯积和：{{
                            StoneCrackDetect.block_data[block].crack_data
                              .crackArea
                          }}</el-row
                        >
                        <el-row
