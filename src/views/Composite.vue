  ...
  methods: {
    ...
    handleClick_quick(type) {
      ...
      if (type === "SE")
        //记录旋转的面
        this.rotating = "SE";
      else if (type === "SW") this.rotating = "SW";
    ...
    addSelections(index, datalist, child) {
      //更新选项
      var that = this;
      ...
      if (index === 1) {
        ...
        for (let i = 0; i < datalist.length; i++) {
          ...
          for (let j = 0; j < child[i].length; j++) {
            //0.i_001 => 0.i_001  i_001
            temp_children.push({
              value: child[i][j],
              label: child[i][j].split(".")[1],
            });
          }
          that.selections[index].push({
            value: datalist[i],
    ...
    updateSelections(index) {
      ...
      if (index === 0) {
        ...
        var sel_2 = sel_2_SE;
        if (that.select_1 === "SW-20") sel_2 = sel_2_SW;
        ...
        var sel_3_children = [];
        var datalist = [];
        if (that.select_1 === "SE-20") datalist = data_SE;
        else if (that.select_1 === "SW-20") datalist = data_SW;

        var uniqueSet = new Set(); // 创建一个空Set对象
        for (let i = 0; i < datalist.length; i++) {
          if (datalist[i].state === "0") continue;
          var parts = datalist[i].name.split("_"); // SE-20_01-03F_i_001.jpg   ['SE-20', '01-03F', 'i', '001.jpg']
          if (that.select_2 === parts[1]) {
            // 01-03F
            if (!uniqueSet.has(parts[2])) {
              ...
              sel_3_children[index].push(
                i + "." + parts[2] + "_" + parts[3].split(".")[0]
              );
            }
          }
        }
    ...
    sendSelections() {
      ...
      if (that.select_1 === "SW-20") mess = data_SW[index];
      //alert(mess.x + "," + mess.y + "," + mess.z);
      //this.$refs.unityModel.sendOrders(mess.x + "," + mess.y + "," + mess.z);

      var baseURL =
        "https://zhl-pictures.obs.cn-north-4.myhuaweicloud.com/Pictures/";
      var imageURL =
        baseURL +
        that.select_1 +
        "/" +
